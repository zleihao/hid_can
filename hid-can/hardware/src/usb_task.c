...
USB_MEM_ALIGNX uint8_t read_buffer[HIDRAW_OUT_EP_SIZE];
USB_MEM_ALIGNX uint8_t send_buffer[HIDRAW_IN_EP_SIZE];

#define HID_STATE_IDLE 0
#define HID_STATE_BUSY 1

/*!< hid state ! Data can be sent only when state is idle  */
volatile uint8_t custom_state;
static volatile uint8_t custom_recv_state;
...
void hid_custom_init(uint8_t busid, uintptr_t reg_base)
{
    ...
    usbd_add_interface(busid, usbd_hid_init_intf(busid, &intf0, hid_custom_report_desc, sizeof(hid_custom_report_desc)));
...
extern void HID_CAN_ExecuteCommand(const uint8_t *request);

/* 创建 usb 任务 */
static StaticTask_t usbd_task_tcb;

uint8_t buffer[64] = { 0x02, 0x78, 0x56, 0x45 };

static void usbd_task_handler(void *param)
{
    hid_custom_init(0, 0x50000000UL);

    while (1) {
        if (1 == custom_recv_state) {
            custom_recv_state = 0;
            HID_CAN_ExecuteCommand(&read_buffer[1]);
        }
        vTaskDelay(3);
    }
}
...
{
    xTaskCreate(usbd_task_handler, "usbd_task_handler", 512, NULL, 4, (TaskHandle_t *)&usbd_task_tcb);
}
